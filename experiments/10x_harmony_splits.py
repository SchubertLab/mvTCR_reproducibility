...
adata = sc.read('../mvTCR/data/10x_CD8TC/v8_harmony.h5ad')
adata = AnnData(X=adata.obsm['X_harmony'], obs=adata.obs, obsm=adata.obsm, uns=adata.uns)
...
params_experiment = {
    'study_name': f'harmony_10x_{args.donor}_{args.model}_filtered_{args.filter_non_binder}_split_{args.split}',
    'comet_workspace': None,
    'model_name': args.model,
    'balanced_sampling': 'clonotype',
    'metadata': ['binding_name', 'clonotype', 'donor'],
    'save_path': os.path.join(os.path.dirname(__file__), '..', 'optuna',
                              f'harmony_10x_{args.donor}_{args.model}_split_{args.split}'),
